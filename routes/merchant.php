...
use App\Http\Controllers\Merchant\BulkMoneyTransferController;
...
use App\Http\Controllers\Merchant\QrCodeGenerateController;
...
Route::prefix("merchant")->name("merchant.")->group(function(){
    ...
    Route::controller(ProfileController::class)->prefix("profile")->name("profile.")->middleware('app.mode')->group(function(){
        ...
        Route::get('email/{token}','showEmailFrom')->name('email');
        Route::get('resend/code','resendCode')->name('resend.code');
        Route::post('mail/verify/{token}','mailVerify')->name('mail.verify');
    });

    //bluk money transfer 
    Route::controller(BulkMoneyTransferController::class)->prefix('bulk-money-transfer')->name('bulk.money.transfer.')->group(function(){
        Route::get('/','index')->name('index');
        Route::post('upload','upload')->name('upload');
        Route::get('preview/{identifier}','preview')->name('preview');
        Route::post('submit/{identifier}','submit')->name('submit');
        Route::get('transaction-log-download/{bmt_id}','transactionLogDownload')->name('transaction.log.download');
    });

    //qrcode 
    Route::controller(QrCodeGenerateController::class)->prefix('qr-code-generate')->name('qrcode.generate.')->group(function(){
        Route::get('/','index')->name('index');
    ...
    Route::middleware('module:merchant-api-key')->group(function(){
        Route::controller(DeveloperApiController::class)->prefix('developer/api')->name('developer.api.')->group(function(){
            Route::get('/','index')->name('index');
            Route::post('mode/update','updateMode')->name('mode.update')->middleware('app.mode');
        });
    });
