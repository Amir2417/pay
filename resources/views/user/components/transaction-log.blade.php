    ...
    @forelse ($transactions as $item)
        <div class="dashboard-list-item-wrapper">
                    ...
                    <div class="dashboard-list-user-wrapper">
                        ...
                        <div class="dashboard-list-user-content">
                            ...
                            @elseif ($item->type == payment_gateway_const()::BULKMONEYTRANSFER)
                                <h4 class="title">{{ __("Bulk Money Received form") }} <span class="text--warning">{{ @$item->merchant->full_name }}</span></h4>
                            ...
                            @elseif ($item->type == payment_gateway_const()::TYPETRANSFERMONEY)
                                @if ($item->isAuthUser())
                                    ...
                                    @if ($item->attribute == payment_gateway_const()::SEND)
                                        <h4 class="title">{{ __("Send Money to") }} {{ __(" @" . @$item->details->receiver->username) }} </h4>
                                    @elseif ($item->attribute == payment_gateway_const()::RECEIVED)
                                        <h4 class="title">{{ __("Received Money from") }} {{ __("@" .@$item->details->sender->username) }} </h4>
                                    @endif
                                @endif
                            ...
                            @elseif ($item->type == payment_gateway_const()::SENDREMITTANCE)
                                @if ($item->isAuthUser())
                                    @if ($item->attribute == payment_gateway_const()::SEND)
                                        <h4 class="title">{{ __("Send Remittance to") }} {{ __("@" . $item->details->receiver->firstname.' '.@$item->details->receiver->lastname) }} </h4>
                                    @elseif ($item->attribute == payment_gateway_const()::RECEIVED)
                                        <h4 class="title">{{ __("Received Remittance from") }} {{ __("@" .@$item->details->sender->fullname) }} </h4>
                                    @endif
                                @endif
                            ...
                            @elseif ($item->type == payment_gateway_const()::REQUESTMONEY)
                                @if ($item->isAuthUser())
                                    @if ($item->attribute == payment_gateway_const()::SEND)
                                        <h4 class="title">{{ __("Request Money to") }} {{ __("@" . $item->details->receiver_username) }} </h4>
                                    @elseif ($item->attribute == payment_gateway_const()::RECEIVED)
                                        <h4 class="title">{{ __("Request Money from") }} {{ __("@" . $item->details->sender_username) }} </h4>
                                    @endif
                                @endif
            ...
            <div class="preview-list-wrapper">
                ...
                @if ($item->type != payment_gateway_const()::BULKMONEYTRANSFER )
