...
function files_path($slug)
{
    $data = [
        ...
        'bulk-money-file'     => [
            'path'          => 'frontend/files/bulk-money',
        ],
        'site-section'         => [
...
function totalAdminProfits(){
    ...
    ->whereHas('transactions', function ($query) {
        $query->whereNotIn('type', [PaymentGatewayConst::TYPEADDMONEY, PaymentGatewayConst::TYPEMONEYOUT,PaymentGatewayConst::TYPEADDSUBTRACTBALANCE,PaymentGatewayConst::BULKMONEYTRANSFER]);
    })
    ->sum('total_charge');
...
function sendApiSMS($message,$mobile){
    $basic_settings = BasicSettingsProvider::get();
    if($basic_settings->sms_notification == true){
        $bearer_token       = $basic_settings->sms_config->api_key;
        $base_url           = $basic_settings->sms_config->base_url;
        $sub_account_id     = $basic_settings->sms_config->sub_account_id;
        $source             = $basic_settings->sms_config->source;
        ...
        $response = Http::withHeaders([
            ...
            'Content-Type'  => 'application/json',
        ])->post($base_url . '/' . $sub_account_id . '/messages', [
            'destination' => $mobile,
            'text'        => $message,
            'source'      => $source,
            'encoding'    => 'AUTO',
        ...
        if(isset($result['status']['code'])){
            if($result['status']['code'] == GlobalConst::QUEUED){
                $data   = [
                    'status'        => true,
