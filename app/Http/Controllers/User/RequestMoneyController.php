    ...
    public function submit(Request $request) {
        ...
        try{
            ...
            if( $basic_setting->sms_notification == true){
                ...
                $message = __("Request Money" . " "  . getAmount($charges['request_amount'],4) . ' ' . $charges['sender_currency'] .  ", to " . $receiver->fullname . " " . "Transaction ID: " . $trx_id . ' ' . "Date : " . Carbon::now()->format('Y-m-d') . ' request sent to admin.');
               sendApiSMS($message,@$sender_wallet->user->full_mobile);

                //receiver notifications
                $message = __("Request Money" . " "  . getAmount($charges['request_amount'],4) . ' ' . $charges['sender_currency'] .  ", From " . @$sender_wallet->user->fullname . " " . "Transaction ID: " . $trx_id . ' ' . "Date : " . Carbon::now()->format('Y-m-d') . ' request sent to admin.');
