...
class AuthorizationController extends Controller
{
    ...
    public function showEmailSendOtp($token)
    {
        $data = UserAuthorization::where('token',$token)->first();
        $page_title = __('email Verification');
        return view('user.auth.authorize.send-email-otp',compact("page_title","token","data"));
    }
    //before register
    public function smsOtpSend($token)
    {
        ...
        return view('user.auth.authorize.sms-otp-send',compact("page_title","token","data"));
    }
    ...
    public function smsVerify(Request $request,$token)
    {
        ...
        if($basic_settings->email_verification == true && auth()->user()->email_verified == false){

            $user = auth()->user();
            mailVerificationTemplate($user);
