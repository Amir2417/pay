...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $transactions = Transaction::with(['merchant'])->auth()->latest()->take(5)->get();
    ...
    public function qrScan($qr_code)
    {
        ...
        $qrCode = UserQrCode::where('qr_code',$qr_code)->where(function ($query) {
    ...
    public function merchantQrScan($qr_code)
    {
        ...
        header("Access-Control-Allow-Methods: PUT, GET, POST");
        header("Access-Control-Allow-Headers: Origin, X-Requested-With, Content-Type, Accept");
        $qrCode = MerchantQrCode::where('qr_code',$qr_code)->where(function ($query) {
            $query->whereNull('sender_type')
                  ->orWhere('sender_type', GlobalConst::SENDER_TYPE_USER);
        })
        ->first();
