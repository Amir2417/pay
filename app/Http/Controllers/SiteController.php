...
class SiteController extends Controller
{
    ...
    /**
     * Method for check user unique user name
     * @param $username
     * @param Illuminate\Http\Request $request
     */
    public function checkUserUniqueName(Request $request){
        $validator      = Validator::make($request->all(),[
        ...
        if($user_name || $agent_name || $merchant_name) return response()->json(['own'=>__("The username has already been taken.")]);

        return response($validated['userName']);
     ...
     * @param Illuminate\Http\Request $request
     */
    public function checkAgentUniqueName(Request $request){
        $validator      = Validator::make($request->all(),[
            'userName'  => 'required|string'
        ]);
        if($validator->fails()){
     ...
     * @param Illuminate\Http\Request $request
     */
    public function checkMerchantUniqueName(Request $request){
        $validator      = Validator::make($request->all(),[
            'userName'  => 'required|string'
        ]);
        if($validator->fails()){
