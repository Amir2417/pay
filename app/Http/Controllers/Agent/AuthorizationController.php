...
class AuthorizationController extends Controller
{
    ...
    public function showEmailSendOtp($token)
    {
        $data = AgentAuthorization::where('token',$token)->first();
        $page_title = __('email Verification');
        return view('agent.auth.authorize.send-email-otp',compact("page_title","token","data"));
    }
    //before register
    public function smsOtpSend($token)
    {
        ...
        return view('agent.auth.authorize.sms-otp-send',compact("page_title","token","data"));
    }
    ...
    public function smsVerify(Request $request,$token)
    {
        ...
        if($basic_settings->agent_email_verification == true && auth()->user()->email_verified == false){

            $agent = auth()->user();
            mailVerificationTemplateAgent($agent);
