...
class AuthorizationController extends Controller
{
    ...
    //before register
    public function showEmailSendOtp($token)
    {
        $data = MerchantAuthorization::where('token',$token)->first();
        $page_title = __('Email Verification');
        return view('merchant.auth.authorize.send-email-otp',compact("page_title","token","data"));
    }
    //before register
    public function smsOtpSend($token)
    {
        $data = MerchantAuthorization::where('token',$token)->first();
        $page_title = __('SMS Verification');
        return view('merchant.auth.authorize.sms-otp-send',compact("page_title","token","data"));
    }
    /**
     * Method for show sms verify page.
     * @param $token
     */
    ...
    public function smsVerify(Request $request,$token)
    {
        ...
        if($basic_settings->merchant_email_verification == true && auth()->user()->email_verified == false){

            $merchant = auth()->user();
            mailVerificationTemplateMerchant($merchant);
