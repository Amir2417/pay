...
class RegisterController extends Controller
{
    ...
    */
    ...
    //========================before registration======================================
    /**
     * Method for send merchant verification code.
     */
    public function sendVerifyCode(Request $request){
        ...
        if($validated['register_type'] == GlobalConst::PHONE){
            ...
            return redirect()->route('merchant.sms.verify',$data['token'])->with(['success' => [__('Verification code sended to your phone number.')]]);
        }else{
            ...
            return redirect()->route('merchant.email.send.otp.code',$data['token'])->with(['success' => [__('Verification code sended to your email address.')]]);
        }       
    ...
    public function EmailVerifyCode(Request $request,$token){
        ...
        if($auth_column->created_at->addSeconds($otp_exp_sec) < now()) {
            $auth_column->delete();
            return redirect()->route('merchant.register.kyc')->with(['error' => [__('Session expired. Please try again')]]);
        }
        ...
        try{
            ...
            return redirect()->route('merchant.register.kyc')->with(['error' => [__('Something went wrong! Please try again.')]]);
        }
    ...
    public function smsOtpResendCode(){
        ...
        return redirect()->route('merchant.sms.otp.send',$data['token'])->with(['success' => [__('Verification code resend success')]]);
    }
    /**
     * Method for email resend code
     */
    public function emailOtpResendCode(){
        ...
        return redirect()->route('merchant.email.send.otp.code',$data['token'])->with(['success' => [__('Verification code resend success')]]);
    }
    /**
     * Method for register kyc page
     */
    ...
    public function register(Request $request)
    {
        ...
        if($register_data != null && $register_data['sms_verified'] == true){
            ...
            if($validated['email'] != '' || $validated['email'] != null){
                $exist = Merchant::where('email',$validated['email'])->first();
