...
class SendMoneyController extends Controller
{
    ...
    public function checkUser(Request $request){
        ...
        $column = "full_mobile";
        $exist = Agent::where($column,$request->phone)->first();
        if( !$exist){
        ...
        $data =[
            'receiver_phone'   => $exist->full_mobile,
            'receiver_wallet' => [
    ...
    public function qrScan(Request $request)
    {
        ...
        $data =[
            'receiver_phone'   => $user->full_mobile,
            'receiver_wallet' => [
            ...
            if($sender){
                 ...
                 try{
                    ...
                    if(($auth_user->email_verified == true && $auth_user->sms_verified == true) || ($auth_user->email_verified == true && $auth_user->sms_verified == false)){
                        $notifyDataSender = [
                            'trx_id'  => $trx_id,
                            'title'  => "Send Money to @" . @$receiver_wallet->agent->username." (".@$receiver_wallet->agent->email.")",
            ...
            if($receiverTrans){
                 ...
                 try{
                    ...
                    if(($auth_user->email_verified == true && $auth_user->sms_verified == true) || ($auth_user->email_verified == true && $auth_user->sms_verified == false)){
                       $notifyDataReceiver = [
                           'trx_id'  => $trx_id,
                           'title'  => "Received Money from @" .@$sender_wallet->agent->username." (".@$sender_wallet->agent->email.")",
                           'received_amount'  => getAmount($charges['receiver_amount'], 2).' ' .$charges['receiver_currency'],
                           'status'  => "Success",
                       ];
                       //send notifications
    ...
    public function insertSenderCharges($id,$charges,$sender_wallet,$receiver_wallet) {
        ...
        try{
            ...
            $notification_content['title'] = __('Transfer Money Send To').' ('.$receiver_wallet->agent->username.')';
    ...
    public function insertReceiverCharges($id,$charges,$sender_wallet,$receiver_wallet) {
        ...
        try{
            ...
            //admin notification
            $notification_content['title'] = __('Transfer Money Received From').' ('.$sender_wallet->agent->username.')';
